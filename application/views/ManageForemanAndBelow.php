...
<div class="container mt-3">
    <table class="table table-form">
        <tr>
            <th class="topic-background mit border">Copy AA001-Bonus&Annual Evaluate Foreman & Below - PMIS From</th>
...
<div class="container-fluid mt-3">
    <?php foreach ($bonus_evaluate_foreman_id as $bonus_evaluate_foreman_ids) {
        if (count($bonus_evaluate_foreman_id) != 0) {
            $update_indicator = 1;
        } else {
            $update_indicator = 0;
        }
        ...
        <tr>
            <td class="border mit-v td_border">
                <?php if ($update_indicator == 1) { ?>
                    <input type="hidden" id="up_id" name="up_id" value="<?php echo $bonus_evaluate_foreman_ids->id; ?>">
                    <select name="up_year" id="up_year" class="form-select">
                        <?php
                        $current_year = date('Y');
                        for ($i = $current_year - 5; $i < $current_year + 5; $i++) { ?>
                            <option value="<?php echo $i; ?>" <?php if ($i == $bonus_evaluate_foreman_ids->year) echo "selected"; ?>><?php echo $i; ?></option>
                        <?php } ?>
                    </select>
                <?php } else { ?>
            ...
            <td class="border mit-v td_border">
                <?php if ($update_indicator == 1) { ?>
                    <div class="form-floating">
                        <textarea name="up_evaluation_Item_en" id="up_evaluation_Item_en" class="form-control h-textarea" style="height: 100px;"><?php echo $bonus_evaluate_foreman_ids->evaluation_Item_en; ?></textarea>
            ...
            <td class="border mit-v td_border">
                <?php if ($update_indicator == 1) { ?>
                    <div class="form-floating">
                        <textarea name="up_evaluation_Item_th" id="up_evaluation_Item_th" class="form-control h-textarea" style="height: 100px;"><?php echo $bonus_evaluate_foreman_ids->evaluation_Item_th; ?></textarea>
                        <label for="" class="font-twelve">
                            Please fill in !
                            <span class="red font-twelve">*</span>
                        </label>
            ...
            <td class="border mit-v td_border">
                <?php if ($update_indicator == 1) { ?>
                    <select name="up_status" id="up_status" class="form-select">
                        ...
                        <option value="1" <?php if ($bonus_evaluate_foreman_ids->status == "1") echo "selected"; ?>>Active</option>
                        <option value="0" <?php if ($bonus_evaluate_foreman_ids->status == "0") echo "selected"; ?>>Inactive</option>
                    </select>
                <?php } else { ?>
...
<div class="container mt-3">
    <fieldset>
        <legend class="mx-2 mt-2 h2">Search Bonus & Annual Evaluate Foreman & Below</legend>
    </fieldset>
...
<div class="container mt-3">
    <table class="table table-form">
        ...
        <?php foreach ($bonus_evaluate_foreman as $bonus_evaluate_foremans) {
            $k++; ?>
            <tr>
                ...
                <td class="border mit">
                    <?php if ($bonus_evaluate_foremans->status == '1') {
                        echo "Active";
                    } else {
                        echo "Inactive";
                    } ?>
                ...
                <td class="border mit">
                    <a href="<?php echo base_url('index.php/manage_Self_Evaluation/ManageForemanAndBelow/?id=') ?><?php echo $bonus_evaluate_foremans->id ?>">Edit</a>
                </td>
            </tr>
        <?php } ?>
    </table>
