...
class Bonus_controller extends CI_Controller
{
    ...
    function FormBonusAnnualEvaluateG4G6()
    {
        ...
        $this->load->view('include/header', $title);
        $this->load->view('include/menu');
        $this->load->view('FormBonusAnnualEvaluateG4G6', $bonus_evaluate_g4_g6 + $emp_data + $emp_leave_data + $data_EvaluateG4G6_id + $period_time);
        $this->load->view('include/footer');
    }
    ...
    function FormBonusAnnualEvaluateG2G3()
    {
        ...
        $period_time['period_time'] = $this->hr->model_period_time();
        $data_EvaluateG2G3_id['data_EvaluateG2G3_id'] = $this->hr->model_data_EvaluateG2G3_id($id);
        $emp_data['emp_data'] = $this->hr->model_emp_data($emp_id);
        $emp_leave_data['emp_leave_data'] = $this->hr->model_emp_leave_data($emp_id);
        ...
        $this->load->view('include/header', $title);
        $this->load->view('include/menu');
        $this->load->view('FormBonusAnnualEvaluateG2G3', $bonus_evaluate_g2_g3 + $emp_data + $emp_leave_data + $data_EvaluateG2G3_id + $period_time);
        $this->load->view('include/footer');
    }
    ...
    function FormBonusAnnualEvaluateForemanAndbelow()
    {
        ...
        $this->load->view('include/header', $title);
        $this->load->view('include/menu');
        $this->load->view('FormBonusAnnualEvaluateForemanAndbelow', $bonus_evaluate_Foreman_below + $emp_data + $emp_leave_data + $data_EvaluateForeman_id + $period_time);
        $this->load->view('include/footer');
    }
    ...
    function Submit_bonus_evaluate_g2_g3_ajax()
    {
        ...
        $creative = $this->input->post('creative');
        $management_mind = $this->input->post('management_mind');
    ...
    function up_Submit_bonus_evaluate_g2_g3_ajax()
    {
        $up_id = $this->input->post('up_id');
        $up_date_submit = $this->input->post('up_date_submit');
        $up_emp_name = $this->input->post('up_emp_name');
        ...
        $up_potential = $this->input->post('up_potential');
        $up_effectiveness = $this->input->post('up_effectiveness');
        $up_planning = $this->input->post('up_planning');
        $up_preventive = $this->input->post('up_preventive');
        $up_creative = $this->input->post('up_creative');
        $up_management_mind = $this->input->post('up_management_mind');
        $up_problem_solving = $this->input->post('up_problem_solving');
        $up_assessment_score_h = $this->input->post('up_assessment_score_h');
        $up_leave_score_h = $this->input->post('up_leave_score_h');
        $up_total_score_h = $this->input->post('up_total_score_h');
        $up_assessment_status = $this->input->post('up_assessment_status');
        ...
        $rs = $this->hr->model_up_Submit_bonus_evaluate_g2_g3($up_id, $up_date_submit, $up_emp_name, $up_emp_id, $up_position, $up_section, $up_hired_date, $up_emp_year_of_service, $up_sup_name1, $up_sup_name2, $up_Factory_Manager_GM, $up_quality_of_work, $up_job_responsibility, $up_cooperation, $up_communication, $up_teamwork, $up_potential, $up_effectiveness, $up_planning, $up_preventive, $up_creative, $up_management_mind, $up_problem_solving, $up_assessment_score_h, $up_leave_score_h, $up_total_score_h, $up_assessment_status, $up_year_submit, $up_business_leave1, $up_business_leave2, $up_sick_leave1, $up_sick_leave2, $up_absenteeism1, $up_absenteeism2, $up_total_leave1, $up_total_leave2, $up_grand_total, $up_late1, $up_late2, $up_verbal_warning, $up_letter_warning);
        if ($rs) {
            $json = '{"ok": true}';
        } else {
            $json = '{"ok": false}';
    ...
    function StaticFormBonusAnnualEvaluateForemanAndbelow()
    {
        ...
        $this->load->view('include/header', $title);
        $this->load->view('include/menu');
        $this->load->view('StaticFormBonusAnnualEvaluateForemanAndbelow', $bonus_evaluate_Foreman_below + $data_EvaluateForeman_id + $period_time);
        $this->load->view('include/footer');
    }
    ...
    function StaticFormBonusAnnualEvaluateG4G6()
    {
        ...
        $this->load->view('include/header', $title);
        $this->load->view('include/menu');
        $this->load->view('StaticFormBonusAnnualEvaluateG4G6', $bonus_evaluate_g4_g6 + $data_EvaluateG4G6_id + $period_time);
        $this->load->view('include/footer');
    }

    function StaticFormBonusAnnualEvaluateG2G3()
    {
        if (isset($_GET['id'])) {
            $id = $_GET['id'];
        } else {
        ...
        $this->load->view('include/header', $title);
        $this->load->view('include/menu');
        $this->load->view('StaticFormBonusAnnualEvaluateG2G3', $bonus_evaluate_g2_g3 + $data_EvaluateG2G3_id + $period_time);
        $this->load->view('include/footer');
    }
    ...
    public function PrintPDFG2G3($id)
    {
        $data_EvaluateG2G3_id['data_EvaluateG2G3_id'] = $this->hr->model_data_EvaluateG2G3_id($id);
        $bonus_evaluate_g2_g3['bonus_evaluate_g2_g3'] = $this->hr->model_bonus_evaluate_g2_g3();

        $html = $this->load->view('PrintPDFG2G3', $data_EvaluateG2G3_id + $bonus_evaluate_g2_g3, true);
        $mpdf = new \Mpdf\Mpdf();
        $mpdf->WriteHTML($html);

        $title['title'] = 'File PDF BONUS & ANNUAL Assessment for Grade 2 – Grade 3';
        $this->load->view('include/header', $title);
        $this->load->view('include/menu');
        $this->load->view('FilePDFG2G3');
        $this->load->view('include/footer');
    }
}
